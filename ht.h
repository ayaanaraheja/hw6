...
{
    double load = static_cast<double>(numitems + numdeleted) / CAPACITIES[mIndex_];
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    if (mIndex_ + 1 >= sizeof(CAPACITIES) / sizeof(HASH_INDEX_T)) {
        throw std::logic_error("No more capacities left to resize.");
    }
    ++mIndex_;
    ...
    for (size_t i = 0; i < CAPACITIES[mIndex_ - 1]; ++i) {
        if (old[i] != nullptr && !old[i]->deleted) {
            insert(old[i]->item);  // Rehashing with insert() ensures size updates and correct probing
        }
        delete old[i];
    }
}
